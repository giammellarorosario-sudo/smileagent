...
const { generateInstagramPost, generateFacebookPost } = require('../config/ai');
...
// ============================================
// POST Genera Post Facebook (AI)
// ============================================
router.post('/api/social/generate-facebook', isAuthenticated, async (req, res) => {
  const studioId = req.session.studioId;
  const { topic, tone } = req.body;

  try {
    console.log(`📘 Generazione post Facebook: topic="${topic}", tone="${tone}"`);
    ...
    // Genera post Facebook con AI
    const result = await generateFacebookPost(topic, tone || 'professionale');
    ...
    console.error('Errore generazione post Facebook:', error);
    res.status(500).json({
      success: false,
      error: 'Errore durante la generazione del post Facebook'
    });
  }
});

// ============================================
// GET Calendario Editoriale Condiviso
// ============================================
router.get('/api/social/calendar', isAuthenticated, async (req, res) => {
  const studioId = req.session.studioId;

  try {
    // Carica eventi calendario con post associati
    const calendarEvents = db.prepare(`
      SELECT
        sc.*,
        sp.contenuto,
        sp.immagine_url,
        sp.piattaforma as post_platform
      FROM social_calendar sc
      LEFT JOIN social_posts sp ON sc.post_id = sp.id
      WHERE sc.studio_id = ?
      ORDER BY sc.data_programmata ASC
    `).all(studioId);
...
// ============================================
// POST Programma Post nel Calendario
// ============================================
router.post('/api/social/calendar/schedule', isAuthenticated, async (req, res) => {
  const studioId = req.session.studioId;
  const { postId, piattaforma, dataProgrammata, note } = req.body;

  try {
    // Valida input
    if (!postId || !piattaforma || !dataProgrammata) {
      return res.status(400).json({
        success: false,
        error: 'postId, piattaforma e dataProgrammata richiesti'
      });
    }

    // Inserisci evento calendario
    const insertEvent = db.prepare(`
      INSERT INTO social_calendar
      (studio_id, piattaforma, post_id, data_programmata, stato, note)
      VALUES (?, ?, ?, ?, 'programmato', ?)
    `);
    ...
    db.prepare(`
      UPDATE social_posts
      SET stato = 'programmato', data_programmata = ?
      WHERE id = ? AND studio_id = ?
    `).run(dataProgrammata, postId, studioId);

    res.json({
      success: true,
      eventId: result.lastInsertRowid,
      message: 'Post programmato con successo'
    });

  } catch (error) {
    console.error('Errore programmazione post:', error);
    res.status(500).json({
      success: false,
      error: 'Errore durante la programmazione del post'
    });
  }
});
